...
for name in ${ProtocolName[@]}; do
    ...
    do
    ns 154.tcl $defaultTopo $node $defaultFlow $name $defaultSpeed $defaultPacket $defaultTx || {
    ...
    do
    ns 154.tcl $defaultTopo $defaultNode $flow $name $defaultSpeed $defaultPacket $defaultTx|| {
    ...
    do
    ns 154.tcl $defaultTopo $defaultNode $defaultFlow $name $speed $defaultPacket $defaultTx || {
    ...
    do
    echo $txRange
    ns 154.tcl $defaultTopo $defaultNode $defaultFlow $name $defaultSpeed $defaultPacket $txRange || {
        echo "ERROR $defaultTopo $defaultNode $defaultFlow $name $defaultSpeed $defaultPacket $txRange" >> error.txt 
    }
    awk -f parse.awk trace.tr  >> data-$name.csv
    done
done
python plot2.py 5 "${ProtocolName[*]}" 154